...
interface BasicsCommandContextHandler {
    @Throws(BasicsCommandException::class)
    fun requirePlayer(name: String): Player {
        val player = Bukkit.getPlayer(name) ?: throw BasicsCommandException(CommandResult.playerNotFound(name))
        return player
    }
    ...
    fun requirePlayerOrMustSpecifyPlayerFromConsole(sender: CommandSender): Player {
        val player = sender as? Player ?: throw CommandResult.MUST_BE_PLAYER_OR_SPECIFY_PLAYER_FROM_CONSOLE.asException()
        return player
    }
