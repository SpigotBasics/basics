...
private val builtinShortenedPackages = mapOf(
...
private fun compactClassName(className: String, shortenedPackages: Map<String, String>): String {
...
fun Throwable.toCompactStackTrace(shortenedPackages: Map<String, String> = emptyMap(), maxLines: Int = 10): String {
    ...
    run loop@{
        this.stackTrace.forEachIndexed { index, element ->
            ...
            compactStackTrace.append("<gray>${packageName}.</gray><gold>${className}</gold><gray>#</gray>${element.methodName}<gray>(</gray>${element.lineNumber}<gray>)</gray>\n")
